...
class ApiService {
  ...
  async validateConfiguration(config: Configuration): Promise<ValidationResult> {
    // For now, mock the API call using our validation service
    // In production, this would call the actual Django backend
    try {
      const { validationService } = await import('./validation-simple');
      ...
      const serviceResult = await validationService.validateConfiguration(validationConfig);
      
      // Convert ValidationService result to API result format
      return {
        ...
        errors: serviceResult.errors.map(error => ({
          ...
          structureIds: error.affectedNodes,
          severity: 'ERROR' as const
        })),
        warnings: serviceResult.warnings.map(warning => ({
          id: warning.id,
          message: warning.description,
          structureIds: warning.affectedNodes,
          severity: 'WARNING' as const
        })),
        suggestions: serviceResult.suggestions.map(suggestion => ({
          id: suggestion.id,
          message: suggestion.description,
          structureIds: suggestion.affectedNodes,
